...
- 指定したページを削除して新しいPDFを書き出す（例: 5ページのPDFからページ5を削除して、1,2,3,4のみのPDFを保存する）
...
- **ページ編集**
  - 「✂️ ページ編集」モードを選択し、PDFファイルを選択します。
  - 「🔢 ページ数を確認」ボタンを押すと、選択中のPDFの総ページ数が表示されます。
  - 削除したいページ番号を1始まりで入力します。複数指定や範囲指定もできます。
    - 例: `5` → ページ5を削除
    - 例: `1,3` → ページ1とページ3を削除
    - 例: `2-4` → ページ2〜4を削除
  - 確定ボタンを押すと、指定ページを削除した新しいPDFが元のファイルと同じフォルダに保存されます（ファイル名に `_edited` が付加されます）。
  - 全ページを削除する指定や、範囲外のページ番号はエラーになります。
